  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            className={`globe3d-marker${hovered ? ' globe3d-marker-hovered' : ''}`}
            onMouseEnter={handlePointerEnter}
            onMouseLeave={handlePointerLeave}
            onClick={handleClick}
          >
            <img src={marker.src} alt={marker.label || 'Marker'} draggable={false} />
          </div>
        </Html>
      </group>
    </group>
...
function LoadingFallback() {
  return (
    <Html center>
      <div className="globe3d-loading">
        <span className="globe3d-loading-text">Loading globe...</span>
      </div>
    </Html>
  )
}
...
export function Globe3D({ markers = [], config = {}, className = '', onMarkerClick, onMarkerHover }) {
  ...
  return (
    <div className={`globe3d-container${className ? ` ${className}` : ''}`}>
      <Canvas
        gl={{
          antialias: true,
          alpha: true,
